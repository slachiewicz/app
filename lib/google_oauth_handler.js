...
var emails = process.env.EMAILS_ALLOWED.split(',');
console.log('EMAISL ', emails)
module.exports = function custom_handler(req, reply, tokens, profile) {

  if(profile) {
    if(emails.indexOf(profile.emails[0].value) === -1 ){
      return reply.redirect('/permission').code(401);
    }else{
