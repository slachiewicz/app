    ...
    config: {
      ...
      handler: function (request, reply) {
        ...
        es.search({
          ...
          size: perPage,
          fields: ['id', 'picture','fullname', 'current', 'location', 'connectedTo', 'favourite', 'contacts.email'],
          query: {
              match_all: {}
          }
            ...
            response.hits.hits.forEach(function (profile) {
              ...
              contact.initials = initials_helper(contact.connectedTo);
              if (profile.fields['contacts.email']) {
                contact.email = profile.fields['contacts.email'][0];
              } else {
                contact.email = '';
              }
